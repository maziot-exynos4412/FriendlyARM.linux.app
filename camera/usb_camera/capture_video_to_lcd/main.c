...
static int LcdOpen(unsigned char *fbname)
{
        ...
        /* 将LCD屏的地址映射到DDR内存空间 返回值LCD屏的首地址 */
        ...
        memset(fbmem, 0x00, fix.smem_len);              // 清屏函数往映射的地址填充 fix.sem_len 大小黑色颜色进去
        return 0;
}
...
static int camera_init(void)
{
        ...
        /* 3、VIDIOC_ENUM_FMT 查询支持哪种格式---枚举出摄像头支持的所有格式 */
        ...
        /* 分配4个buffer:实际上由VIDIOC_REQBUFS 获取到的信息来决定 */
...
void *camera_pthread(void * arg)
{
        ...
        while (1)
        {
                ...
                /*地址赋值 pucVideBuf[ListNum]: 存放摄像头输出的数据 */
...
{
        pthread_t camerathread;                 // 存放线程的 ID
        LcdOpen("/dev/fb0");                    // 打开 lcd
